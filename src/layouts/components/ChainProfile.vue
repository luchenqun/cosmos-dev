  ...
  <div class="dropdown">
    <label tabindex="0" class="flex items-center">
      ...
      <div class="flex-1 w-0">
        ...
        >
          {{ baseStore.latest?.block?.header?.height ? `#${baseStore.latest.block.header.height}` : chainStore.chainName || '' }}
    ...
    <div tabindex="0" class="dropdown-content -left-6 w-80 menu shadow bg-base-200 rounded-box overflow-auto">
      ...
      >
        <div class="flex flex-col">
          <div class="flex items-center justify-between w-full">
            ...
            <span v-if="item.address === chainStore.endpoint?.address" class="bg-yes inline-block h-2 w-2 rounded-full" />
          </div>
      ...
      <div class="w-full">
        <div class="py-2 px-4">Chain Id: {{ baseStore.latest.block?.header.chain_id && baseStore.connected ? baseStore.latest.block.header.chain_id : 'N/A' }}</div>
        <div class="py-2 px-4">Height: {{ baseStore.latest.block?.header.height && baseStore.connected ? baseStore.latest.block.header.height : '0' }}</div>
      </div>
      <!-- bottom-->
