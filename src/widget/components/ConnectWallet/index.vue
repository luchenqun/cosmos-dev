...
import { WalletName, createWallet, removeWallet, writeWallet } from '../../wallet/Wallet';
import type { Account, ConnectedWallet} from '../../wallet/Wallet';
import { readWallet } from '../../wallet/Wallet';
...
<template>
  <div class="mb-4">
    ...
    <div v-if="connected.cosmosAddress" class="dropdown dropdown-hover ping-connect-dropdown">
      <label tabindex="0" class="btn btn-sm m-1 lowercase">{{ connected.wallet }}-{{ connected.cosmosAddress?.substring(connected.cosmosAddress?.length - 4) }}</label>
      <div tabindex="0" class="dropdown-content menu shadow p-2 bg-base-100 rounded w-64 overflow-auto">
        ...
        <div class="">
          ...
          <a class="block py-2 px-2 hover:bg-gray-100 dark:hover:bg-[#353f5a] rounded cursor-pointer" style="overflow-wrap: anywhere" @click="copyAdress(connected.cosmosAddress)">
          ...
          <!-- <div class="divider mt-1 mb-1"></div>
                    ...
                    > -->
          <a class="block py-2 px-2 hover:bg-gray-100 dark:hover:bg-[#353f5a] rounded cursor-pointer" @click="disconnect()">Disconnect</a>
        </div>
      </div>
    ...
    <label v-if="!connected.cosmosAddress" for="PingConnectWallet" class="btn btn-sm ping-connect-btn capitalize">Connect Wallet</label>
