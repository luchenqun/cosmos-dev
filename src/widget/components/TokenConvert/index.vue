...
import type { Asset, Chain, IBCInfo, IBCPath } from '@ping-pub/chain-registry-client/dist/types';
import { getBalance, getStakingParam, getOsmosisPools, getLatestBlock, getAccount, getTxByHash } from '../../utils/http';
import { WalletName, createWallet, readWallet, writeWallet } from '../../wallet/Wallet';
import type { Account, ConnectedWallet} from '../../wallet/Wallet';
...
async function initData() {
  ...
  if (open.value) {
    ...
    getBalance(OSMOSIS_REST, osmoAddress(sender.value.cosmosAddress)).then((res) => {
      osmoBalances.value = res.balances.filter((x) => !x.denom.startsWith('gamm'));
    });
    getBalance(props.endpoint, localAddress(sender.value.cosmosAddress)).then((res) => {
...
function findlocalCoinDecimal(denom: string) {
  ...
  if (localCoin) {
    const unit = localCoin.denom_units.find((x) => x.denom === localCoin.symbol.toLowerCase());
...
function findTokenUrl(coinInfo?: Asset) {
  const url = coinInfo?.logo_URIs?.svg || coinInfo?.logo_URIs?.png || coinInfo?.logo_URIs?.jpeg;
  return url ? url.replace('https://raw.githubusercontent.com/cosmos/chain-registry/master', 'https://registry.ping.pub') : '';
}

const localTokenOnOsmosis = computed(() => {
  const channel = osmosisPathInfo.value.channels?.find((x) => x.chain_1.port_id === 'transfer');
  const channelId = osmosisPathInfo.value.chain_1?.chain_name === 'osmosis' ? channel?.chain_1.channel_id : channel?.chain_2.channel_id;
  const ibcDenom = toHex(sha256(new TextEncoder().encode(`transfer/${channelId}/${defaultDenom.value}`))).toUpperCase();
...
const localSourceChannelID = computed(() => {
  const channel = osmosisPathInfo.value.channels?.find((x) => x.chain_1.port_id === 'transfer');
  const channelId = osmosisPathInfo.value.chain_1?.chain_name === 'osmosis' ? channel?.chain_2.channel_id : channel?.chain_1.channel_id;
  return channelId;
});
...
const pool = computed(() => {
  ...
  const a = allPools.value
    .filter((x) => x.pool_assets?.findIndex((a) => a.token.denom === swapIn.value?.ibcDenom) > -1)
    .filter((x) => x.pool_assets?.findIndex((a) => a.token.denom === swapOut.value?.ibcDenom) > -1)
    .sort((a, b) => Number(b.total_shares.amount) - Number(a.total_shares.amount));
  // console.log(a)
...
const outAmount = computed(() => {
  ...
  const p = pool.value;
  if (p) {
    const tokenBalanceOut = p.pool_assets?.find((x) => x.token.denom === swapOut.value?.ibcDenom);
    const tokenBalanceIn = p.pool_assets?.find((x) => x.token.denom === swapIn.value?.ibcDenom);
    if (tokenBalanceIn && tokenBalanceOut) {
      ...
      const out = balanceOut * (1 - balanceIn / (balanceIn + amount) ** (weightIn / weightOut));
      return out;
    }
  }
  return 0;
...
async function doSwap() {
  ...
  try {
    ...
    const response = await stargateClient.signAndBroadcast(address, [msg], fee, 'Token Convert from ping.pub');
    if (response.code === 0) {
      await showResult(response.transactionHash);
      await getBalance(OSMOSIS_REST, osmoAddress(sender.value.cosmosAddress)).then((res) => {
        osmoBalances.value = res.balances.filter((x) => !x.denom.startsWith('gamm'));
      });
    } else {
  ...
  try {
    ...
    if (response.tx_response?.code === 0) {
      setTimeout(async () => {
        ...
        await getBalance(props.endpoint, localAddress(sender.value.cosmosAddress)).then((res) => {
          localBalances.value = res.balances;
        });
      }, 6000);
    }
      ...
      <label class="modal-box dark:bg-[#2a2a3a] rounded-lg" for="">
        <div v-show="view === 'swap'">
          <div class="absolute right-4 top-4 dropdown dropdown-end dropdown-hover">
            ...
            <div tabindex="0" class="card compact dropdown-content dark:bg-info-content bg-slate-300 shadow rounded-box w-64 z-40">
              <div class="card-body">
                <ul class="text-right">
          ...
          <div class="flex items-center relative h-14 bg-gray-100 dark:bg-[#232333] rounded-tl-lg rounded-tr-lg mt-4">
            <div class="dropdown">
                ...
                <div class="py-2">
                  ...
                  >
                    ...
                    <div class="text-sm font-semibold text-gray-600">
                      {{ showBalance(item.ibcDenom || item.denom, item.decimals) }}
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <input v-model="amountIn" type="number" placeholder="1" class="input bg-transparent flex-1 h-14 text-right text-lg font-bold" />
          </div>
          ...
          <div class="flex items-center justify-center -mt-3 -mb-3">
            <div class="inline-block px-4 cursor-pointer" @click="switchDirection">
              <Icon icon="mdi:arrow-down-circle" color="#676cf6" class="text-4xl dark:bg-gray-50 rounded-full origin-center duration-300 transform hover:rotate-180" />
            </div>
          </div>
          ...
          <div class="flex items-center h-14 rounded-tl-lg rounded-tr-lg bg-gray-100 dark:bg-[#232333]">
            <div v-if="outTokens.length === 0">
              ...
              <button v-else class="btn btn-ghost" :class="{ 'loading relative start-0': sending }">loading...</button>
            </div>
          ...
          <div class="flex items-center py-2 px-4 bg-gray-200 dark:bg-[#171721] rounded-bl-lg rounded-br-lg">
            ...
            <Icon v-if="withdrawable" icon="mdi:minus-box-outline" class="ml-2 cursor-pointer" @click="switchView('withdraw')" />
          </div>

          <div class="px-4 mt-4">
            <div class="flex items-center justify-between">
              <div class="text-sm text-gray-600 dark:text-gray-400">Swap Fee</div>
              <div class="text-base text-gray-800 dark:text-gray-200">{{ decimal2percent(pool?.pool_params.swap_fee) }}%</div>
            </div>
          </div>
        ...
        <div v-show="view === 'deposit'">
          ...
          <div class="form-control">
            ...
            <input :value="osmoAddress(sender.cosmosAddress)" readonly type="text" class="input border border-gray-300 dark:border-gray-600" />
          </div>
          <div class="flex items-center relative h-14 bg-gray-100 dark:bg-[#232333] rounded-tl-lg rounded-tr-lg mt-4">
            <label tabindex="0" class="flex items-center h-12 px-4 cursor-pointer">
              <img :src="swapIn?.coinImageUrl" class="w-8 h-8 mr-3 rounded-full" />
            ...
            <input v-model="depositAmount" type="number" placeholder="1" class="input bg-transparent flex-1 h-14 text-right text-lg font-bold" />
          </div>

          <div class="flex flex-row-reverse items-center py-2 px-4 bg-gray-200 dark:bg-[#171721] rounded-bl-lg rounded-br-lg">
            <div class="text-base font-semibold">
          ...
          <div class="mt-5 flex">
            ...
            <button class="btn btn-primary grow ping-connect-confirm capitalize text-base" :disabled="disableDeposit" @click="doDeposit">
              <span v-if="sending" class="loading loading-spinner"></span>
              Deposit
            </button>
          </div>
        ...
        <div v-show="view === 'withdraw'">
          ...
          <div class="flex justify-between j items-center py-2 px-4 bg-gray-200 dark:bg-[#171721] rounded-tl-lg rounded-tr-lg mt-4">
          ...
          <div class="flex items-center relative h-14 bg-gray-100 dark:bg-[#232333] rounded-bl-lg rounded-br-lg">
            ...
            <input v-model="withdrawAmount" type="number" placeholder="1" class="input bg-transparent flex-1 h-14 text-right text-lg font-bold" />
          </div>
          ...
          <div class="mt-5 flex">
            ...
            <button class="btn btn-primary grow ping-connect-confirm capitalize text-base" :disabled="disableWithdraw" @click="doWithdraw()">
              <span v-if="sending" class="loading loading-spinner"></span>
              Withdraw
            </button>
          </div>
        ...
        <div v-show="view === 'submitting'">
          <div class="mt-10 mb-6">
            ...
            <div class="overflow-hidden h-5 mb-2 text-xs flex rounded bg-green-100">
              <div :style="`width: ${step}%`" class="shadow-none flex flex-col text-center whitespace-nowrap text-white justify-center bg-green-400"></div>
            </div>
            <div class="flex items-center justify-between">
              <div>
                <span class="text-xs font-semibold inline-block py-1 px-2 rounded text-gray-600 dark:text-white"> Submitted </span>
              </div>
              <div class="text-right">
                <span class="text-xs font-semibold inline-block text-gray-600 dark:text-white"> {{ step }}% </span>
              </div>
            </div>
          </div>
