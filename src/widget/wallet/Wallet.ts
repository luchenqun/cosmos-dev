...
export function readWallet(hdPath?: string) {
  return JSON.parse(localStorage.getItem(hdPath || DEFAULT_HDPATH) || '{}') as ConnectedWallet;
}
...
export function createWallet(name: WalletName, arg: WalletArgument, registry?: Registry): AbstractWallet {
  ...
  switch (name) {
    ...
    case WalletName.Metamask:
      return arg.hdPath && (arg.hdPath.startsWith('m/44/60') || arg.hdPath.startsWith("m/44'/60")) ? new MetamaskWallet(arg, reg) : new MetamaskSnapWallet(arg, reg);
  }
