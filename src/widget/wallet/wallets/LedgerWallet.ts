...
import { WalletName, extractChainId, keyType } from '../Wallet';
import { AminoTypes, createAuthzAminoConverters } from '@cosmjs/stargate';
...
export class LedgerWallet implements AbstractWallet {
  ...
  hdPath: string;
  registry: Registry;
  aminoTypes = new AminoTypes({ ...createAuthzAminoConverters(), ...createWasmAminoConverters() });
  conf: WalletArgument;
  constructor(arg: WalletArgument, registry: Registry) {
    this.name = WalletName.Ledger;
    this.transport = arg.transport || 'usb';
    this.hdPath = arg.hdPath as string; // || DEFAULT_HDPATH;
  ...
  async getSigner() {
    ...
    return new LedgerSigner(transport, { ledgerAppName, hdPaths: [hdPath as any] });
  }
  ...
  async sign712(tx: Transaction): Promise<Uint8Array> {
    ...
    const toSignTx = generateMessageWithMultipleTransactions(sender.accountNumber.toString(), sender.sequence.toString(), tx.signerData.chainId, tx.memo, fees, msgs);
